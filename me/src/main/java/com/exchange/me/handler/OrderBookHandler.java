...
public class OrderBookHandler {
   ...
   *
   * <p>This method is the main entry point of the matching engine. It validates that the order
   * belongs to the current trading pair and delegates the matching process based on the order type
   * and trade side.
   *
   * <p>Supported order types:
   *
   * <ul>
   *   <li>LIMIT: Matches against available liquidity at acceptable prices. Any remaining quantity
   *       may be added to the order book.
   *   <li>MARKET: Executes immediately against the best available prices without considering a
   *       limit price.
   *   <li>FOK (Fill Or Kill): Executes only if the entire order quantity can be filled immediately.
   *       If full execution is not possible, the order is cancelled and no partial execution
   *       occurs.
   * </ul>
   *
   * @param timestamp the matching event timestamp
   * @param incomingOrder the order submitted to the matching engine
   * @return a list of executed matches generated from this order. Returns an empty list when the
   *     order cannot be executed (for example, an unfillable FOK order).
   * @throws InvalidTradePairException if the order trading pair does not match this order book's
   *     trading pair
   */
  public List<MatchInfo> matchOrder(long timestamp, Order incomingOrder) {
    ...
    /**
     * FOK (Fill Or Kill) orders require a limit price because the engine must verify that the
     * entire order quantity can be filled within the acceptable price range before execution.
     *
     * <p>A FOK order without a valid price cannot determine whether full execution is possible.
     */
    if (incomingOrder.getOrderType() == OrderType.FOK && incomingOrder.getPrice() <= 0) {
      throw new IllegalArgumentException("FOK order requires a valid limit price");
    }

    return switch (incomingOrder.getOrderType()) {
      case LIMIT, MARKET ->
          incomingOrder.getTradeSide() == TradeSide.BUY
  ...
  public List<MatchInfo> executeBuyOrder(long timestamp, Order buyOrder) {
    ...
    while (!asks.isEmpty() && buyOrder.getRemainingQuantity() > 0) {
      ...
      /**
       * Stops matching when the best available ask price is higher than the maximum price accepted
       * by a LIMIT BUY order.
       *
       * <p>MARKET orders ignore price limits and continue matching against available liquidity.
       * LIMIT orders only execute trades at prices equal to or better than their specified limit
       * price.
       */
      if (buyOrder.getOrderType() == OrderType.LIMIT && askPrice > buyOrder.getPrice()) {
        break;
      }
      ...
      while (!askList.isEmpty() && buyOrder.getRemainingQuantity() > 0) {
        ...
        if (askOrder == null) break;

        long tradedQuantity =
            Math.min(buyOrder.getRemainingQuantity(), askOrder.getRemainingQuantity());
     ...
     *
     * <p>If the order could not be fully matched and still has remaining quantity, it is stored in
     * the bid side of the book. MARKET orders are never added because they must execute immediately
     * or expire.
     */
    if (buyOrder.getOrderType() == OrderType.LIMIT && buyOrder.getRemainingQuantity() > 0) {
  ...
  public List<MatchInfo> executeSellOrder(long timestamp, Order sellOrder) {
    ...
    while (!bids.isEmpty() && sellOrder.getRemainingQuantity() > 0) {
      ...
      /**
       * Stops matching when the best available bid price is lower than the minimum price accepted
       * by a LIMIT SELL order.
       *
       * <p>A LIMIT SELL order can only match with buyers offering a price equal to or higher than
       * the seller's limit price.
       */
      ...
      while (!bidList.isEmpty() && sellOrder.getRemainingQuantity() > 0) {
        ...
        if (bidOrder == null) break;

        long tradedQuantity =
            Math.min(sellOrder.getRemainingQuantity(), bidOrder.getRemainingQuantity());
     ...
     *
     * <p>If the order is partially filled and still has remaining quantity, it is placed on the ask
     * side of the order book for future matching.
     */
    if (sellOrder.getOrderType() == OrderType.LIMIT && sellOrder.getRemainingQuantity() > 0) {
  ...
  private List<PriceLevel> queueDepth(int levels, TreeMap<Long, Deque<Order>> orders) {
    ...
    for (Map.Entry<Long, Deque<Order>> entry : orders.entrySet()) {
      ...
      double volume = entry.getValue().stream().mapToDouble(Order::getRemainingQuantity).sum();
  ...
  private boolean canFullyFill(Order order) {
    ...
    if (order.getOrderType() == OrderType.FOK && order.getPrice() <= 0) {
      throw new IllegalArgumentException("FOK order requires a valid limit price");
    }
